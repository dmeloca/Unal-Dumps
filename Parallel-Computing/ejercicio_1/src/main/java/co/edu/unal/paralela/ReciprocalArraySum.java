...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        private static final int SEQUENTIAL_THRESHOLD = 2000; // Seuil à ajuster
        ...
        protected void compute() {
            ...
            if (length <= SEQUENTIAL_THRESHOLD) {
                // Calcul séquentiel pour les petites tâches
                for (int i = startIndexInclusive; i < endIndexExclusive; i++) {
                    value += 1 / input[i];
                }
            } else {
                // Division récursive pour les grandes tâches
                ...
                left.fork();        // Lance left en parallèle
                right.compute();    // Exécute right dans le thread courant
                left.join();        // Attend la fin de left
                
                value = left.getValue() + right.getValue(); // Combine les résultats
                }
        }
    }
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        double sum = 0;
        int n = getChunkSize(numTasks, input.length);
