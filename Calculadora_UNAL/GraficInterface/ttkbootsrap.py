...
def mostrar_materias(nombre_carrera):
    ...
    for materia in carrera:
        ...
        label_materia.pack( anchor='s', pady=2)
        label_materia.bind('<Button-1>', lambda e,
                           m=materia: agregar_materia_seleccionada(m))
    # Botón para agregar nueva materia
    boton_agregar_materia = ttk.Button(frame_materias, text="Agregar Materia", command=agregar_nueva_materia)
    boton_agregar_materia.pack(side='bottom', pady=10, padx=10, fill='x')
...
def mostrar_calculadora_unal():
    ...
    def calculadorUnal(nombreSegundaCarrera):
        ...
        frame=ttk.Frame(ventana_calculadora_unal)
        frame.pack(side='bottom')
        ttk.Separator(frame, orient='horizontal').pack(side='top', fill='x')
        tk.Label(frame, text=f"creditos Necesarios: {creditosNecesariosV}",font=('Calibri', 20, 'bold') ).pack()
        print('creditos necesarios',creditosNecesariosV)
        tk.Label(frame, text=f"creditos Adicionales: {creditos_adicionales}",font=('Calibri', 20, 'bold') ).pack()
        mensaje=doble_titulacion(promedio, porcentaje_avance, creditosNecesariosV, creditos_adicionales)
        mostrarMensaje(mensaje, frame)

    def mostrarMensaje(mensaje, frame):
        if mensaje:
            #con letras grandes y de color verde
            tk.Label(frame, text="FELICIDADES, usted es elegible para hacer doble titulación", fg="green", font=('Calibri', 26, 'bold')).pack()

        else:
            #con letras grandes y de color rojo
            tk.Label(frame, text="Lo sentimos, no es elegible para hacer doble titulación", fg="red", font=('Calibri', 26, 'bold')).pack()
    ...
    framel=ttk.Frame(ventana_calculadora_unal)
    framel.pack(side="left")
    # subtitulo
    ttk.Label(framel, text="Elija la carrera con la cual quiere hacer la doble: ", font=('Calibri', 16, 'bold')).pack(anchor='center')
    ...
    for carrera in segundas_carreras:
        boton_carrera = ttk.Button(framel, text=carrera , command=lambda c=carrera: calculadorUnal(c) )
    ...
    framer=ttk.Frame(ventana_calculadora_unal)
    framer.pack()
    framer1=ttk.Frame(framer)
    framer1.pack(side="left")
    ttk.Label(framer1, text=f"Promedio: {promedio}",font=('Calibri', 18, 'bold')).pack()
    ttk.Label(framer1, text=f"Avance de carrera: {porcentaje_avance:.2f}%",font=('Calibri', 18, 'bold')).pack()
    ttk.Label(framer1, text=f"Créditos adicionales: {creditos_adicionales}",font=('Calibri', 18, 'bold')).pack()
    ttk.Label(framer1, text="Materias homologables:",font=('Calibri', 18, 'bold')).pack()
...
ttk.Label(root, text='Calculadora UNAL', font='Calibri 20 bold').pack()
style = ttk.Style()
...
frame_carreras.pack(side='top', fill='both', pady=20)
...
frame_materias_seleccionadas.pack(side='left', fill='x', pady=20)
...
label_creditos_adicionales = ttk.Label(root, text="Créditos adicionales: 0", font='Calibri 14', justify='left')
label_creditos_adicionales.pack(side='top')

# Etiqueta para mostrar el promedio
lbl_promedio = ttk.Label(root, text="Promedio: 0", font='Calibri 14')
lbl_promedio.pack(side='top', pady=0)

# Etiqueta para mostrar el avance de la carrera
lbl_avance = ttk.Label(root, text="Avance de la carrera: 0%", font='Calibri 14')
lbl_avance.pack(side='top', pady=0)

# Cuadro de texto para los créditos cancelados
label_creditos_cancelados = ttk.Label(root, text="Ingrese el número de créditos que canceló:", font='Calibri 14')
label_creditos_cancelados.pack(side='top')
#entrada créditos cancelados
entrada_creditos_cancelados = ttk.Entry(root)
entrada_creditos_cancelados.pack(side='top')
# Botón "OK" para confirmar los créditos cancelados
boton_confirmar = ttk.Button(root, text="OK", command=confirmar_creditos_cancelados)
boton_confirmar.pack(side='top', pady=5, padx= 5)
##
boton_calculadora_unal = ttk.Button(root, text="Calcular", command=mostrar_calculadora_unal)
boton_calculadora_unal.pack(side='bottom',pady=10, fill='x')
...
ttk.Label(frame_carreras, text='Seleccione su pregrado: ', font='Calibri 14').pack(side='top')
for carrera in carreras:
    boton = ttk.Button(frame_carreras, text=carrera,command=lambda c=carrera: mostrar_materias(c))
    boton.pack(side= 'top',padx=10, pady=5)
# Separador vertical
